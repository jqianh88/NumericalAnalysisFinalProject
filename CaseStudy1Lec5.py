...
p = 2
    ...
    while i<N:                                  # loops
        pNew  = a+(b-a)/2
        diffC = diffB                           # calculate the 2 back
        diffB = diffA                           # calculate the 1 back
        diffA = abs(pNew-p)                     # most recent
        p = pNew
        if f1(p) == 0:
            break
        if f1(a)*f1(p) > 0:
    ...
    ax.scatter(range(N),alphas=100,linewidths=2.5, facecolors='none', edgecolors='b')
    ax.axis('equal')
